...
public class SmsSendServiceImpl implements SmsSendService {
    ...
    public SmsResultModel sendSmsByTemplate(SmsRequestModel requestModel) {
        ...
        SmsCompany smsCompany = bootFinishListener.getSmsCompanyByTag(CompanyTag.valueByTag(sdktag));
        //数据封装
    ...
    public SmsResultModel sendSmsWithoutTemplate(String phoneNum, String content) {
        ...
        LogModel logModel = new LogModel();
        logModel.setlSendTime(new Date());
        logModel.setlContent(content);
        logModel.setlPhoneNum(phoneNum);
        logModel.setlTempId(-1);
        logModelMapper.insertSelective(logModel);

        SmsCompany smsCompany = bootFinishListener.getSmsCompanyByTag(CompanyTag.YI_MEI_RUAN_TONG);
        return smsCompany.send(phoneNum,content);
    }
}
